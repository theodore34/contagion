...
def main():
    ...
    # ── Tableaux R² -> CSV ────────────────────────────────────────────────────
    crise_df = selection.crisis_table(intervals_chrono, period_data, ctx, crisis_map,
                                      csv_path=config.RESULTS_DIR / "actifs_crise_R2.csv")
    print(f"{len(crise_df)} lignes (actif x periode) -> results/actifs_crise_R2.csv")
    pk_df, peak_curves = selection.peak_table(pk["peak_rises"], pk["peak_pd"], ctx,
    ...
    print(selection.redundancy_recap(period_data, pk["peak_pd"], ctx).to_string())
    ...
    plots.plot_three_views(selection.collect_by_method(period_data, selected),
                           name="courbes_3vues_abs",
                           suptitle="Resultat 2 : 3 vues par methode (|C|)")
    plots.plot_three_views(selection.collect_by_method(sig["period_data_sig"], sig["selected_sig"]),
                           name="courbes_3vues_signe", label_suffix=" [signe]",
                           suptitle="Les 3 vues avec C_ij signe (detection signee)")
    plots.plot_three_views(selection.collect_by_method(pk["peak_pd"], pk["peak_rises"]),
    ...
    # ── Comparaison des périodes retenues (avec/sans pics) + λ_max/⟨λ⟩ ─────────
    W_crise = selection.retain(selection.window_summaries(intervals_chrono, period_data, ctx.all_days))
    W_pics = selection.retain(selection.window_summaries(pk["peak_rises"], pk["peak_pd"], ctx.all_days))
    plots.plot_period_comparison(ctx.mc_all, pk["peak"]["mc_s"], ctx.all_days, ctx.mean_mc,
                                 W_crise, W_pics, spec["diag"], spec["lam_dates"],
                                 spec["mix_periods"])
    nc_c, ns_c = selection.totals(W_crise)
    nc_p, ns_p = selection.totals(W_pics)
