"""Standalone rolling contagion pipeline.

Reads one or more CSVs of prices (one date column + one column per asset),
inner-joins them on date, converts to log-returns, builds a correlation-based
sparsity mask, fits per-window VAR contagion matrices with a constant, and
caches the result.

Usage
-----
    from rolling_contagion_standalone import rolling_contagion
    res = rolling_contagion("stock_filled.csv", corr_quantile=0.8, asset_type="stock")
    res = rolling_contagion(["a_filled.csv", "b_filled.csv"],
                            corr_quantile=0.8, asset_type="mixed")
"""
